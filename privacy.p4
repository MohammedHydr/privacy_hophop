...
control MyIngress(inout headers hdr,
    ...
    action encrypt_c2s() {
        if (hdr.ipv4.isValid() && hdr.tcp.isValid()) {
            // Only encrypt client flows with dynamic/private TCP source ports [49152, 65535]
            if (hdr.tcp.srcPort >= 16w49152 && hdr.tcp.srcPort <= 16w65535) {
                ...
                if ((hdr.ipv4.srcAddr == CLIENT_IP && hdr.ipv4.dstAddr == SERVER_IP) ||
                    ...
                    hash(nonce, HashAlgorithm.crc32, (bit<32>)0,
                         { hdr.ipv4.dstAddr, hdr.tcp.dstPort,
                    ...
                    bit<32> pad;
                    hash(pad, HashAlgorithm.crc32, (bit<32>)0,
                         { PRIV_KEY, nonce }, (bit<32>)0xffffffff);
